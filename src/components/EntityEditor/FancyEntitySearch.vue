        ...
        >
          <q-item :key="item.id" clickable dense @click="$emit('entitySelected', item)">
            <q-item-section avatar>
              <q-icon
                :name="getIcon(item)"
                :title="getIconTooltip(item)"
                :class="{ restriction: isRestricted(item) }"
              />
            </q-item-section>
            <q-item-section>
              ...
              <template v-if="showDetails">
                <q-item-label
                  v-for="(description, index) in getDescriptions(item)"
                  :key="index"
                  caption
                  class="ellipsis"
                >
...
async function loadOptions(page = 1): Promise<EntityPage> {
  if (!entityApi) return Promise.reject({ message: t('fetchDataFailed') })
  if (!isValid.value) return Promise.resolve(emptyPage)
