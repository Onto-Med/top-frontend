<template>
  <q-page>
    <q-splitter
      ...
      style="min-height: inherit; height: 100px"
      :limits="[15, 50]"
    >
      <template #before>
        <div class="column fit">
          <div
            v-if="repository"
            class="row items-center q-pl-sm bg-primary text-white shadow-2 entity-editor-tabs-bar"
          >
            ...
            <q-btn-group flat class="gt-xs">
              ...
              <q-btn
                v-if="writable && env.QUERIES_ENABLED"
                dense
                :icon="isConceptRepository ? 'find_in_page' : 'manage_search'"
                :title="t('buildQuery')"
                :to="{
                  name: 'queryBuilder',
                  params: { organisationId, repositoryId: repository.id },
                }"
              />
            </q-btn-group>
        ...
        <div class="column fit">
          ...
          >
            <q-tab
              ...
              :key="tab.state.id"
              :name="tab.state.id"
              @dblclick="tab.preserve = true"
            >
              <span class="no-wrap" :class="{ 'text-italic': !tab.preserve }">
                <q-icon
                  :name="getIcon(tab.state)"
                  :class="{ restriction: isRestricted(tab.state) }"
                />
                {{ getTabTitle(tab) }}
...
const { entities, isAuthenticated, repository, organisationId, organisation } =
...
onMounted(() => {
  ...
  reloadEntities()
    .then(() => {
      ...
      return entityStore.loadEntity(props.entityId)
    })
    .then((entity) => {
      if (!entity) return
...
onBeforeRouteLeave(
  async () =>
    await confirmCloseUnsavedTabs()
      .then(() => true)
      .catch(() => false),
)

watch(repository, (newVal) => {
  if (!newVal || !repository.value || newVal.id !== repository.value.id) closeTab(undefined)
})
    ...
    .then(
      () =>
        (tabs.value = tabs.value.filter(
          (t) => entities.value.findIndex((e) => e.id === t.state.id) !== -1,
        )),
    )
    .catch((e: Error) => renderError(e))
...
function switchTab(offset: number) {
  let index = tabs.value.findIndex((t) => selected.value?.id === t.state.id)
  if (index === -1) return
  index += offset
...
function restoreVersion(entity: Entity) {
  entityStore
    ...
    .then(() => {
      ...
      if (index !== -1 && tabs.value[index]) tabs.value[index].state = entity
    })
    .catch((e: Error) => renderError(e))
}
...
async function confirmCloseUnsavedTabs(tab?: EditorTab) {
  return new Promise<void>((resolve, reject) => {
    if ((tab && !tab.dirty && !!tab.selectedVersion) || !hasUnsavedTabs.value) return resolve()
    $q.dialog({
      component: Dialog,
      componentProps: {
