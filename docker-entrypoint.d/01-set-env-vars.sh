...
then
    sed -i "s|MAX_COMBINED_DOCUMENTS_UPLOAD\\:!1|MAX_COMBINED_DOCUMENTS_UPLOAD:${MAX_COMBINED_DOCUMENTS_UPLOAD}|g" /usr/share/nginx/html/assets/*.js
fi

if [[ ! -z "${ACCEPT_DOCUMENT_UPLOAD_TYPE}" ]]
then
    sed -i "s|ACCEPT_DOCUMENT_UPLOAD_TYPE\\:!1|ACCEPT_DOCUMENT_UPLOAD_TYPE:${ACCEPT_DOCUMENT_UPLOAD_TYPE}|g" /usr/share/nginx/html/assets/*.js
fi
